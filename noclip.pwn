...
public OnFilterScriptExit()
{
	...
	{
		if(noclipdata[x][cameramode] == CAMERA_MODE_FLY) CancelEditMode(x);
	}
	return 1;
}
...
{
	if(!strcmp(cmdtext, "/noclip", true))
	{
	    // Place the player in and out of edit mode
		if(GetPVarType(playerid, "EditMode")) CancelEditMode(playerid);
		else EditMode(playerid);
		return 1;
	}
	return 0;
}
	...
	if(noclipdata[playerid][cameramode] == CAMERA_MODE_FLY)
	{
		...
		if(noclipdata[playerid][udold] != ud || noclipdata[playerid][lrold] != lr)
		{
			...
			{   // Indicates a new key has been pressed
				...
				GetMoveDirectionFromKeys(ud, lr, noclipdata[playerid][mode]);
...
stock GetMoveDirectionFromKeys(ud, lr, &direction)
{
    if(lr < 0)
	{
...
stock GetNextCameraPosition(keys, Float:CP[3], Float:FV[3], &Float:X, &Float:Y, &Float:Z)
{
    // Calculate the cameras next position based on their current position and the direction their camera is facing
